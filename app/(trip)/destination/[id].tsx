    ...
    <View className="flex-1 bg-bg-primary">
      ...
      <Animated.ScrollView onScroll={scrollHandler} scrollEventThrottle={16} contentContainerClassName="pb-24">
        ...
        <Animated.View style={[headerStyle, { height: HEADER_HEIGHT }]}>
          <CachedImage source={{ uri: destination.image }} className="w-full h-full" resizeMode="cover" />
        ...
        <View className="flex-row px-6 mb-4">
          {[{ iconName: 'cash', label: 'Budget', value: destination.priceIndex < 0.4 ? 'Low' : destination.priceIndex < 0.7 ? 'Medium' : 'High' },
            { iconName: 'thermometer', label: 'Weather', value: '28°C' },
            { iconName: 'airplane', label: 'Flight', value: '5h 30m' }
          ].map((stat) => (
        ...
        <View className="px-6 mb-6">
          ...
          <View className="flex-row gap-3">
            <TouchableOpacity className="flex-1 bg-bg-card border border-border py-3 rounded-button items-center" onPress={() => router.push('/(trip)/flights')}>
              <Text className="text-white text-body-sm">Flights</Text>
            </TouchableOpacity>
        ...
        <View className="px-6">
          ...
          {activities.slice(0, 4).map((act) => (
            <TouchableOpacity key={act.id} className="flex-row bg-bg-card border border-border rounded-card p-3 mb-2" onPress={() => router.push(`/(trip)/activity/${act.id}`)}>
              <CachedImage source={{ uri: act.image }} className="w-20 h-20 rounded-lg mr-3" />
