    ...
    <ScrollView className="flex-1 bg-bg-primary" contentContainerClassName="pt-safe pb-24">
      ...
      <Animated.View entering={FadeInDown.delay(300)} className="mx-6 mb-4">
        ...
        <View className="flex-row flex-wrap gap-2">
          {[
            { emoji: '🗺️', label: 'Map', route: '/(live)/map' },
            { emoji: '💰', label: 'Expense', route: '/(live)/expenses' },
            { emoji: '📸', label: 'Photo', route: '/(live)/memories' },
            { emoji: '🎫', label: 'Tickets', route: '/(live)/checkin' },
            { emoji: '🆘', label: 'Emergency', route: '/(live)/emergency' },
            { emoji: '💱', label: 'Currency', route: '/(trip)/currency' },
          ].map((action) => (
          ...
          <TouchableOpacity key={act.id} className="flex-row bg-bg-card border border-border rounded-card p-3 mb-2" onPress={() => router.push(`/(live)/activity/${act.id}`)}>
            ...
            <View className="justify-center">
              {i === 0 ? (
                ...
                <View className="bg-primary/20 px-2 py-1 rounded-full"><Text className="text-primary text-caption">Done ✓</Text></View>
              ) : null}
            </View>
          </TouchableOpacity>
        ))}
