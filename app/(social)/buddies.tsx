...
const BUDDIES = [
  ...
  { id: '2', name: 'Mike R.', avatar: '👨', status: 'offline', lastTrip: 'Tokyo', sharedTrips: 1 },
  { id: '3', name: 'Emma L.', avatar: '👱‍♀️', status: 'online', lastTrip: 'Bali', sharedTrips: 2 },
  { id: '4', name: 'David W.', avatar: '🧔', status: 'traveling', lastTrip: 'Iceland', sharedTrips: 0 },
  { id: '5', name: 'Lisa M.', avatar: '👩‍🦰', status: 'online', lastTrip: 'Paris', sharedTrips: 5 },
];

const REQUESTS = [
  { id: 'r1', name: 'Tom H.', avatar: '👦', match: 85, mutual: 2 },
  { id: 'r2', name: 'Anna P.', avatar: '👧', match: 78, mutual: 1 },
];
      ...
      {tab === 'friends' ? (
        <FlatList
          ...
          <TouchableOpacity onPress={() => router.push(`/(social)/profile/${item.id}`)} className="flex-row items-center mx-4 mb-2 p-4 bg-bg-secondary rounded-2xl border border-white/[0.08]">
            ...
            <View className="flex-1">
              ...
              <Text className="text-white/40 text-xs">{item.status === 'traveling' ? '✈️ Currently traveling' : `Last trip: ${item.lastTrip}`}</Text>
            </View>
        ...
        <FlatList data={REQUESTS} keyExtractor={i => i.id} renderItem={({ item }) => (
          <View className="flex-row items-center mx-4 mb-2 p-4 bg-bg-secondary rounded-2xl border border-white/[0.08]">
            ...
            <TouchableOpacity className="bg-white/[0.05] px-3 py-2 rounded-xl"><Text className="text-white/60 text-sm">✕</Text></TouchableOpacity>
          </View>
        )} />
      )}
    </View>
