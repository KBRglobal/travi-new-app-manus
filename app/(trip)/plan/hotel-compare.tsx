...
function getVal(hotel: typeof HOTELS[0], attr: string) {
 ...
 case 'Pool': return hotel.pool ? 'checkmark' : 'close';
 case 'Breakfast': return hotel.breakfast ? 'checkmark' : 'close';
 case 'Cancellation': return hotel.cancel;
 case 'Distance center': return hotel.distance;
 case 'WiFi': return hotel.wifi ? 'checkmark' : 'close';
 default: return '';
 }
}
...
export default function HotelCompare() {
 ...
 <Text className=" text-xl font-[Satoshi-Bold]" style={{ color: colors.text.primary }}>Compare Hotels (3)</Text>
 </View>
 ...
 <View className="bg-[#120824] rounded-[16px] p-2 w-full items-center">
 <Text className=" font-[Satoshi-Bold] text-sm" style={{ color: colors.text.primary }}>{h.name}</Text>
 </View>
 </View>
 ))}
 </View>
 ...
 <View style={{ width: 120 }}><Text className="text-[rgba(255,255,255,0.6)] text-sm">{attr}</Text></View>
 {vals.map((v, i) => (
 <View key={i} style={{ width: 110 }} className="items-center">
 <Text className=" text-sm" style={{ color: colors.text.primary }}>{v}</Text>
 </View>
 ))}
 </View>
 );
 ...
 <View className="bg-[#6443F4] rounded-[12px] px-3 py-2"><Text className=" text-xs font-[Satoshi-Bold]" style={{ color: colors.text.primary }}>Select</Text></View>
 </TouchableOpacity>
 ))}
 </View>
 </View>
 ...
 <TouchableOpacity onPress={() => router.push('/(trip)/plan/hotels')} className="bg-[#120824] rounded-[12px] py-3 items-center mb-8" style={{ borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' }}>
 <Text className="text-[#6443F4] font-semibold">+ Add Hotel</Text>
 </TouchableOpacity>
 </ScrollView>
 </View>
 );
