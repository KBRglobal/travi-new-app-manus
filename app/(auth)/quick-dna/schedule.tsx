    ...
    <View style={{ flex: 1, backgroundColor: colors.bg.primary }} pt-safe">
      ...
      <View className="flex-row items-center px-6 md:px-12 mt-4">
        ...
        <View className="flex-1 h-1 bg-white/10 rounded-full ml-4">
          <View className="w-3/4 h-full bg-[#6443F4] rounded-full" />
        </View>
      </View>

      <ScrollView contentContainerClassName="px-6 md:px-12 pb-32">
        <Text className="text-2xl md:text-3xl font-[Satoshi-Bold]  mt-6" style={{ color: colors.text.primary }}>How do you plan?</Text>
        <Text className="text-sm md:text-base text-[rgba(255,255,255,0.6)] mt-2">There's no wrong answer</Text>
        ...
        <View className="gap-3 mt-6 max-w-md">
          {OPTIONS.map((opt) => (
            <Pressable
              ...
              className={`h-18 flex-row items-center px-4 rounded-[16px] border ${
                selected === opt.id
                  ? 'bg-[#6443F4]/20 border-[#6443F4]'
                  : 'bg-white/5 border-white/8'
              }`}
            >
              <Text className="text-2xl mr-4">{opt.icon}</Text>
              <View className="flex-1">
                <Text className=" text-base font-[Satoshi-Bold]" style={{ color: colors.text.primary }}>{opt.title}</Text>
                <Text className="text-[rgba(255,255,255,0.3)] text-xs">{opt.subtitle}</Text>
              </View>
              {selected === opt.id && (
                <View className="w-5 h-5 rounded-full bg-[#6443F4] items-center justify-center">
      ...
      <View className="absolute bottom-0 left-0 right-0 px-6 md:px-12 pb-safe mb-4 bg-bg-[#6443F4] pt-4">
        <Pressable
          ...
          className={`w-full max-w-md mx-auto h-14 rounded-[12px] items-center justify-center ${
            selected ? 'bg-[#6443F4] active:opacity-80' : 'bg-[#6443F4] opacity-40'
          }`}
        >
