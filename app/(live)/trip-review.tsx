    ...
    <View style={{ flexDirection: "row", gap: 6 }}>
      {[1, 2, 3, 4, 5].map((star) => (
        <TouchableOpacity key={star} onPress={() => onRate(star)} activeOpacity={0.7}>
          <Text style={{ fontSize: size, color: star <= rating ? "#FFD700" : "rgba(255,255,255,0.2)" }}>★</Text>
        </TouchableOpacity>
      ))}
    </View>
  );
        ...
        <View style={S.doneRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={{ flexDirection: "row", gap: 4, marginTop: 2 }}>
              {[1, 2, 3, 4, 5].map((s) => (
                <Text key={s} style={{ fontSize: 14, color: s <= item.rating ? "#FFD700" : "rgba(255,255,255,0.15)" }}>★</Text>
              ))}
            </View>
          </View>
...
const S = StyleSheet.create({
  ...
  ptsCounterText: { color: "#A78BFA", fontSize: 13, fontWeight: "800" },
  ...
  reviewCard: { borderRadius: 20, padding: 16, gap: 12, overflow: "hidden", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" },
  reviewCardDone: { borderColor: "rgba(34,197,94,0.3)" },
  reviewHeader: { flexDirection: "row", alignItems: "center", gap: 12 },
  reviewTitle: { color: "#FFF", fontSize: 16, fontWeight: "700" },
  reviewCategory: { color: "#5A4D72", fontSize: 12, marginTop: 2 },
  noteInput: { backgroundColor: "rgba(255,255,255,0.05)", borderRadius: 12, padding: 12, color: "#FFF", fontSize: 14, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", minHeight: 60, textAlignVertical: "top" },
  submitBtn: { borderRadius: 14, overflow: "hidden" },
  submitBtnDisabled: { backgroundColor: "rgba(255,255,255,0.04)" },
  ...
  doneNote: { color: "#9BA1A6", fontSize: 13, fontStyle: "italic" },

  completeWrap: { flex: 1, alignItems: "center", justifyContent: "center", gap: 16, paddingHorizontal: 32 },
  completeTitle: { color: "#FFF", fontSize: 28, fontWeight: "900", textAlign: "center" },
  completeSub: { color: "#9BA1A6", fontSize: 15, textAlign: "center", lineHeight: 22 },
  pointsBadge: { borderRadius: 16, overflow: "hidden" },
  pointsBadgeGrad: { paddingHorizontal: 24, paddingVertical: 12 },
  pointsBadgeText: { color: "#FFF", fontSize: 15, fontWeight: "800" },
  completeTip: { color: "#5A4D72", fontSize: 13, textAlign: "center", lineHeight: 20 },
